...
import { Heart, Building2, Eye, EyeOff, Check, X, AlertCircle } from 'lucide-react';
        ...
        {PASSWORD_RULES.map(r => {
          ...
          return (
            <div key={r.id} style={{
              ...
              fontSize: '.75rem', color: ok ? '#16a34a' : 'var(--ds-on-surface-variant)',
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    // Client-side validation (don't rely solely on server)
    ...
    try {
      ...
      navigate('/verify-otp', {
        state: { userId: data.userId, role: data.role, email: payload.email },
      });
    } catch (err) {
  ...
  const canSubmit = !loading && !pwMismatch && pwAllPassed;
        ...
        <div style={{
          ...
          <div style={{ marginBottom: '1.75rem' }}>
            ...
            <p style={{ color: 'var(--ds-on-surface-variant)', fontSize: '.95rem' }}>
              Join the medicine donation network
            </p>
          </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem' }}>
              {[
                ...
                <div
                  ...
                  tabIndex={0}
                  onKeyDown={e => e.key === 'Enter' && set('role', value)}
                  style={{
                  ...
                  <div style={{
                    width: 36, height: 36,
                    background: form.role === value ? 'var(--ds-secondary-container)' : 'var(--ds-surface-container)',
                    borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto .6rem',
                  }}>
                    <Icon size={18} style={{ color: form.role === value ? 'var(--ds-secondary)' : 'var(--ds-on-surface-variant)' }} />
                  </div>
                  <div style={{ fontFamily: 'Hanken Grotesk, sans-serif', fontWeight: 600, fontSize: '.9rem', color: form.role === value ? 'var(--ds-secondary)' : 'var(--ds-primary)', marginBottom: '.2rem' }}>
                    {label}
                  </div>
          ...
          <form onSubmit={handleSubmit} noValidate>
            <div style={{ marginBottom: '1rem' }}>
              <label style={labelStyle}>
                {form.role === 'ngo' ? 'Contact Person Name' : 'Full Name'}{' '}
            ...
            {form.role === 'ngo' && (
              <div style={{ marginBottom: '1rem' }}>
                ...
                <input
                  style={inputStyle} type="text" placeholder="NGO or organization name"
            ...
            <div style={{ marginBottom: '1rem' }}>
              ...
              <input
                style={inputStyle} type="text" placeholder="City, State"
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ position: 'relative' }}>
                ...
                />
                <button
                  type="button" onClick={() => setShowConfirmPw(p => !p)}
          ...
          <div style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '.875rem', color: 'var(--ds-on-surface-variant)' }}>
            ...
            <Link to="/login" style={{ color: 'var(--ds-secondary)', fontWeight: 600, textDecoration: 'none' }}>
              Sign in
            </Link>
          </div>
        </div>
